...
import { Currency, Currencies } from 'store/types'
...
export function Dropdown({ label, ...rest }: DropdownProps) {
  ...
  const { isOpen, toggleOpen, handleSelect, currenciesList, pocketValue, selected, pocketValueColor, handleOnKeyUp } = useDropdown(label, ref)
        ...
        {isOpen && (
          ...
          >
            ...
            {filtered.map((item: Currency, idx: number) => {
              return <CurrencyItem key={`${item.name}-${idx}`} handleOnKeyUp={handleOnKeyUp} handleSelect={handleSelect} item={item} />
            })}
          </Box>
        )}
      </Box>
