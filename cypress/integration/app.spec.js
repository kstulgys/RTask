...
describe('Currency exchange app', () => {
  ...
  it('When typing in currency "FROM" input - the pocket value "FROM", input value "TO" and pocket value "TO" changes as expected', () => {
    user.findByTestId("pocket-from").invoke('text').then((pocketValueFrom) => {
      ...
      user.findByTestId("pocket-from").invoke('text').then((nextPocketValue) => {
        expect(nextPocketValue).to.eq(formatValueToSpecial(expectedValue))
      })
      // input value "TO" changes automaticaly
      user.findByTestId('input-to').invoke('val').then((inputValueTo) => {
        user.findByTestId("current-rate").invoke('text').then((currentRate) => {
          const expected = (+currentRate * Number(inputValueFrom)).toFixed(2)
          expect(inputValueTo).to.eq(expected)
        })
      })
      // pocket value "TO" increases
      user.findByTestId("pocket-to").invoke('text').then((pocketValueTo) => {
        user.findByTestId("input-to").invoke('text').then((inputValueTo) => {
  ...
  it('When typing in currency "To" input - the pocket value "To", input value "FROM" and pocket value "FROM" changes as expected', () => {
    user.findByTestId("pocket-from").invoke('text').then((pocketValueFromOld) => {
      console.log({pocketValueFromOld})
      user.findByTestId("pocket-to").invoke('text').then((pocketValueTo) => {
        let _inputValueTo = '2385.49'
        ...
        user.findByTestId("pocket-to").invoke('text').then((nextPocketValue) => {
          expect(nextPocketValue).to.eq(formatValueToSpecial(expectedValue))
        })
        // input value "From" changes automaticaly
        user.findByTestId('input-to').invoke('val').then((inputValueTo) => {
          user.findByTestId("current-rate").invoke('text').then((currentRate) => {
            user.findByTestId("input-from").invoke('val').then((inputValueFrom) => {
        ...
        user.findByTestId("input-from").invoke('val').then((inputValueFrom) => {
          user.findByTestId("pocket-from").invoke('text').then((pocketValueFromNew) => {
